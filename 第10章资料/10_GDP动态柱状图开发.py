...
for line in data_lines:
    ...
    # 如何判断字典里面有没有指定的key呢？
    try:
        data_dict[year].append([country, gdp])  # 如果有，就直接添加
    except KeyError:
        data_dict[year] = []
        data_dict[year].append(
            [country, gdp]
        )  # 如果没有，就创建一个新的key，并添加数据
...
# 排序年份
sorted_year_list = sorted(data_dict.keys())  # 取出年份的key，进行排序

# for循环每一年的数据，基于每一年的数据，创建每一年的bar对象
for year in sorted_year_list:
    # 对每一年的数据进行排序，按照gdp从大到小进行排序
    ...
    for country_gdp in year_data:
        ...
        y_data.append(country_gdp[1] / 100000000)  # y轴添加gdp数据，除以1亿，单位为亿
